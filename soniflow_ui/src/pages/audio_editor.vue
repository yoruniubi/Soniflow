...
const goBack = async () => {
  ...
  const pathParts = currentPath.value.split(/[/\\]/); // Split by / or \
  // Remove empty strings from split (e.g., for root path or trailing slash)
  const filteredParts = pathParts.filter(part => part !== '');
  
  if (filteredParts.length <= 1) { // If only one part (e.g., "VocalCraft" or empty after filter)
    await fetchDirectory('/'); // Go to root if current is a top-level directory
  } else {
    const parentPath = '/' + filteredParts.slice(0, filteredParts.length - 1).join('/');
...
}, { immediate: false }); // Do not run immediately on mount, only on changes

onMounted(() => {
  // Fetch initial directory listing
  fetchDirectory('/VocalCraft');
...
<template>
  <div class="editor-container">
    ...
    <div class="file-panel">
      <div class="path-bar">
        <v-btn icon @click="goBack" :disabled="currentPath === '/' || currentPath === '/VocalCraft'">
