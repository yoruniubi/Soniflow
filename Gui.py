...
class Api:
    ...
    def _sanitize_filename(self, filename):  
        ...
        # Remove or replace filename中的非法字符  
        # Keep only valid characters for filenames (alphanumeric, spaces, dots, dashes, underscores)
        # This regex is more restrictive but safer for cross-platform compatibility
    ...
    def get_default_output_directory(self):
        """API to get the default output directory from settings."""
        try:
            default_dir = config_manager.get('defaultOutput', os.path.join(os.getcwd(), 'output'))
            return {"success": True, "path": default_dir}
        except Exception as e:
            logging.error(f"Failed to get default output directory: {str(e)}", exc_info=True)
            return {"success": False, "error": str(e)}
    ...
    def open_directory_dialog(self):
        try:
            ...
            if result and len(result) > 0:
                ...
                return {"success": False, "error": "No directory selected or dialog cancelled."}
        except Exception as e:
            logging.error(f"Error opening native directory dialog: {str(e)}", exc_info=True)
            return {"success": False, "error": str(e)}
    ...
    def upload_file_stream(self, file_object):
        ...
        try:
            ...
            sanitized_file_name = self._sanitize_filename(file_name)
            save_path = UPLOADS_DIR / sanitized_file_name
    ...
    def process_audio(self, number, input_filename, output_directory, codec, bitrate):
        ...
        try:
            ...
            # Extract basename, sanitize it, and construct the full path within the upload directory
            base_filename = os.path.basename(input_filename)
            sanitized_base_filename = self._sanitize_filename(base_filename)
            input_path = os.path.join(self.upload_dir, sanitized_base_filename)
    ...
    def _run_spleeter_in_subprocess(self, number, input_path, output_dir, codec, bitrate):
        """子进程执行函数"""
        try:
            ffmpeg_dir = resource_path(os.path.join("ffmpeg"))
            if os.path.isdir(ffmpeg_dir):
                print(f"[Spleeter Subprocess] Found bundled FFmpeg directory: {ffmpeg_dir}")
                original_path = os.environ.get("PATH", "")
                os.environ["PATH"] = ffmpeg_dir + os.pathsep + original_path
                
                print(f"[Spleeter Subprocess] Temporarily prepended FFmpeg dir to PATH.")
                # logging.debug(f"New PATH for subprocess: {os.environ['PATH']}") # for debugging
            else:
                print("[Spleeter Subprocess] WARNING! Bundled FFmpeg directory not found.")
                print("                     Spleeter will rely on the system's global PATH.")
          
            from spleeter_part import SpleeterPart  # 在子进程内导入
            start_time = time.time()
    ...
    def get_cwd(self):
        ...
        try:
            cwd = os.getcwd()
            # Normalize for Windows drive roots
            if sys.platform == 'win32' and len(cwd) == 2 and cwd[1] == ':':
                cwd += os.sep
    ...
    def get_parent_directory(self, path):
        ...
        try:
            ...
            # On Windows, if parent_path is just a drive letter (e.g., 'C:'),
            # os.path.dirname returns 'C:'. We want 'C:\' for consistency.
            if sys.platform == 'win32' and len(parent_path) == 2 and parent_path[1] == ':':
                parent_path += os.sep
            # If it's the root of a drive (e.g., 'C:\'), os.path.dirname returns 'C:\'.
            # If it's the root of a Unix-like system ('/'), os.path.dirname returns '/'.
            # If it's a network path like '\\server\share', os.path.dirname might return '\\server'.
            # For the purpose of going "back", if parent_path is the same as path, it means we are at the root.
            if parent_path == path and path != os.path.abspath(os.sep): # Check if it's not the actual root
                # This means we are at a drive root like 'C:\' and dirname returned itself.
                # For the UI, we might want to signify "no further back" or handle it as a special root.
                # For now, let's return the same path, and frontend can disable the button.
                pass 
            return {"success": True, "path": parent_path}
        except Exception as e:
            logging.error(f"Failed to get parent directory for {path}: {str(e)}", exc_info=True)
            return {"success": False, "error": str(e)}
    ...
    def get_drives(self):
        ...
        if sys.platform == 'win32':
            ...
            # For Unix-like systems, typically just the root is considered a "drive"
            # or mounted volumes would be listed from /Volumes or /media
            # For simplicity, we'll just return the root for now.
            drives.append("/")
    ...
    def form_transformation(self, file_data, output_format): # Renamed file_name to file_data for clarity
        try:
            ...
            # Sanitize the base_name before constructing the output filename
            sanitized_base_name = self._sanitize_filename(base_name)
            output_filename = f"{sanitized_base_name}_converted.{output_format.lower()}"
...
def run_server(port=8000, directory='./soniflow_ui/dist'):
...
def run_webview():
    ...
    @app_flask.route('/uploads/<path:filename>')
    ...
    def _run_flask_local():
        app_flask.run(host='0.0.0.0', port=5000, threaded=True, debug=False, use_reloader=False)

    # 在当前进程中启动 Flask 服务器线程
    flask_local_thread = threading.Thread(target=_run_flask_local)
    flask_local_thread.daemon = True
    flask_local_thread.start()
    logging.info("Flask API 服务器线程已在 webview 进程内启动。")
    time.sleep(1) # 给服务器一点时间来启动
    
    api = Api()
    is_production = whether_production()
    real_url = 'http://localhost:8000/' if is_production else 'http://localhost:3000/'
    ...
    webview.start(debug=False) # debug=True in dev, debug=False in production
...
def force_utf8_for_io():
    """
    强制 sys.stdout 和 sys.stderr 使用 UTF-8 编码。
    这是解决在 Windows 上 console=False 模式下 print() 导致 gbk 错误的关键。
    """
    # 仅在需要时重新配置，避免不必要的操作
    if sys.stdout.encoding != 'utf-8':
        sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8')
        logging.info("Forced stdout to UTF-8.")
    if sys.stderr.encoding != 'utf-8':
        sys.stderr = io.TextIOWrapper(sys.stderr.buffer, encoding='utf-8')
        logging.info("Forced stderr to UTF-8.")
if __name__ == '__main__':
    multiprocessing.freeze_support()
    # 增加日志记录，帮助调试
    logging.info("应用程序启动...")
    # 强制设置UTF-8编码，解决Windows打包后控制台输出问题
    force_utf8_for_io()
    is_production_mode = whether_production()
    logging.info(f"是否为生产模式: {is_production_mode}")
    # --- 修改开始 ---
    # 在生产模式下才需要启动我们自己的静态文件服务器
    if is_production_mode: 
        # 获取打包后前端文件的路径
        dist_path = resource_path('soniflow_ui/dist')
        logging.info(f"生产模式，前端文件路径: {dist_path}")
        
        # 检查路径是否存在，这是一个很好的健壮性措施
    ...
    try:
        ...
        logging.info(f"加载托盘图标: {image_path}")
        image = Image.open(image_path)
        menu = Menu(MenuItem('打开', on_open), MenuItem('退出', on_exit))
        icon = Icon('Soniflow', image, "Soniflow", menu=menu) # 增加一个title参数
